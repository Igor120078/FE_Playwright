...
export class OfferPageYearlySavingsPOM extends POM {
  ...
  private yearlySavingsSufficiencyText: LabelInterface
  private yearlySavingsSufficiencyValue: LabelInterface
  ...
  constructor(page: Page) {
    ...
    this.yearlySavingsOptimumValue = ComponentFactory.createLabel()
      ...
      .setVisible(true)
    this.yearlySavingsSufficiencyText = ComponentFactory.createLabel()
      .label()
      .setLocator(this.page.getByText('FVE_OFFER_TYPE.SELF_SUFFICIENCY', { exact: true }).nth(1))
      .setVisible(true)
    this.yearlySavingsSufficiencyValue = ComponentFactory.createLabel()
      .label()
      .setLocator(this.page.locator("(//div[text()='FVE_OFFER_TYPE.SELF_SUFFICIENCY'])[2]/following-sibling::div"))
      .setVisible(true)
  }
  ...
  async validateAllComponents(): Promise<void> {
    ...
    await this.yearlySavingsSufficiencyText.validateSelf()
    await this.yearlySavingsSufficiencyValue.validateSelf()
  }
  ...
  async validateYearlySavingsValues() {
    ...
    let sufficiencySavings = Number((await this.yearlySavingsSufficiencyValue.readElementText()).replace(/\D+/g, ''))
    console.log(`Yearly savings Sufficiency: ${sufficiencySavings}`)
    expect(sufficiencySavings).toBeGreaterThan(10)
  }
}
